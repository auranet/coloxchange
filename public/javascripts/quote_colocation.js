$(function(){
  var year = $('#quote_target_date_1i'), month = $('#quote_target_date_2i'), day = $('#quote_target_date_3i');
  $('a.calendar').DatePicker({
    date: new Date(),
    onChange: function(dateString) {
      var date = dateString.split('-');
      year.val(date.shift());
      month.val(parseInt(date.shift()).toString());
      day.val(parseInt(date.shift()).toString());
      this.style.display = 'none';
    }
  });
  $('a.calendar').DatePickerSetDate(new Date(parseInt(year.val()), parseInt(month.val()) - 1, parseInt(day.val())), true);
